...
const statuses: Record<VehicleStatus, string> = {
  standby: 'text-[--color-gray-6] bg-surface-2',
  ok: 'text-ok bg-ok-subtle',
  alarm: 'text-alarm bg-alarm-subtle',
  warning: 'text-warning bg-warning-subtle',
  caution: 'text-caution bg-caution-subtle',
}

type HardwareType = 'Skynode X' | 'Skynode GX' | 'Skynode ENT'

const hardwareBadges: Record<HardwareType, string> = {
  'Skynode X': 'text-dim bg-surface-2 ring-line',
  'Skynode GX': 'text-primary-text bg-primary-subtle ring-primary-border',
  'Skynode ENT': 'text-accent-text bg-accent-subtle ring-accent-border',
}
...
<template>
  <div class="h-full bg-page text-high">
    <!-- Mobile sidebar -->
        ...
        <div class="fixed inset-0 flex">
          <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="-translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="-translate-x-full">
            <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1">
              ...
              <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-page px-6 ring-1 ring-line">
                <div class="flex h-16 shrink-0 items-center gap-3">
                  <svg class="h-5 shrink-0 text-high" viewBox="0 0 780 148" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <span class="text-xs font-medium text-low">Suite</span>
                </div>
                <nav class="flex flex-1 flex-col">
                  <ul role="list" class="flex flex-1 flex-col gap-y-7">
                    <li>
                      <ul role="list" class="-mx-2 space-y-1">
                        <li v-for="item in navigation" :key="item.name">
                          <a :href="item.href" :class="[item.current ? 'bg-primary-subtle text-primary' : 'text-low hover:bg-surface-2 hover:text-primary', 'group flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold']">
                            <component :is="item.icon" :class="[item.current ? 'text-primary' : 'text-[--color-gray-6] group-hover:text-primary', 'size-6 shrink-0']" aria-hidden="true" />
                            {{ item.name }}
                          </a>
                        </li>
                      </ul>
                    ...
                    <li>
                      ...
                      <ul role="list" class="-mx-2 mt-2 space-y-1">
                        <li v-for="fleet in fleets" :key="fleet.name">
                          <a :href="fleet.href" :class="[fleet.current ? 'bg-primary-subtle text-primary' : 'text-low hover:bg-surface-2 hover:text-primary', 'group flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold']">
                            <span :class="[fleet.current ? 'border-primary-border text-primary' : 'border-line text-[--color-gray-6] group-hover:border-primary-border group-hover:text-primary', 'flex size-6 shrink-0 items-center justify-center rounded-lg border bg-surface-1 text-[0.625rem] font-medium']">{{ fleet.initial }}</span>
                    ...
                    <li class="-mx-6 mt-auto">
                      <a href="#" class="flex items-center gap-x-4 px-6 py-3 text-sm/6 font-semibold text-high hover:bg-surface-2">
                        <img class="size-8 rounded-full bg-surface-2 outline -outline-offset-1 outline-line" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="" />
    ...
    <div class="xl:pl-72">
      ...
      <div class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-6 border-b border-line bg-page px-4 shadow-xs sm:px-6 lg:px-8">
        <button type="button" class="-m-2.5 p-2.5 text-high xl:hidden" @click="sidebarOpen = true">
        ...
        <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
          <form class="grid flex-1 grid-cols-1" action="#" method="GET">
            <input name="search" aria-label="Search" class="col-start-1 row-start-1 block size-full bg-transparent pl-8 text-base text-high outline-hidden placeholder:text-[--color-gray-6] sm:text-sm/6" placeholder="Search vehicles, missions..." />
      ...
      <main class="lg:pr-96">
        <header class="flex items-center justify-between border-b border-line px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
          <h1 class="text-base/7 font-semibold text-high">Active Missions</h1>

          <!-- Sort dropdown -->
          <Menu as="div" class="relative">
            <MenuButton class="flex items-center gap-x-1 text-sm/6 font-medium text-high">
              Sort by
              <ChevronUpDownIcon class="size-5 text-[--color-gray-6]" aria-hidden="true" />
            </MenuButton>
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform scale-100" leave-to-class="transform opacity-0 scale-95">
              <MenuItems class="absolute right-0 z-10 mt-2.5 w-40 origin-top-right rounded-md bg-surface-2 py-2 shadow-lg outline-1 outline-line">
                <MenuItem v-slot="{ active }">
                  <a href="#" :class="[active ? 'bg-surface-3' : '', 'block px-3 py-1 text-sm/6 text-high']">Vehicle</a>
                </MenuItem>
                <MenuItem v-slot="{ active }">
                  <a href="#" :class="[active ? 'bg-surface-3' : '', 'block px-3 py-1 text-sm/6 text-high']">Last updated</a>
                </MenuItem>
                <MenuItem v-slot="{ active }">
                  <a href="#" :class="[active ? 'bg-surface-3' : '', 'block px-3 py-1 text-sm/6 text-high']">Status</a>
                </MenuItem>
                <MenuItem v-slot="{ active }">
                  <a href="#" :class="[active ? 'bg-surface-3' : '', 'block px-3 py-1 text-sm/6 text-high']">Hardware</a>
                </MenuItem>
              </MenuItems>
            </transition>
          </Menu>
          ...
          <li v-for="mission in missions" :key="mission.id" class="relative flex items-center space-x-4 px-4 py-4 sm:px-6 lg:px-8">
            <div class="min-w-0 flex-auto">
              <div class="flex items-center gap-x-3">
                ...
                <h2 class="min-w-0 text-sm/6 font-semibold text-high">
                  <a :href="mission.href" class="flex gap-x-2">
                    <span class="truncate font-mono text-low">{{ mission.vehicleName }}</span>
              ...
              <div class="mt-3 flex items-center gap-x-2.5 text-xs/5 text-low">
      ...
      <!-- Activity feed -->
      <aside class="bg-surface-1 lg:fixed lg:top-16 lg:right-0 lg:bottom-0 lg:w-96 lg:overflow-y-auto lg:border-l lg:border-line">
        <header class="flex items-center justify-between border-b border-line px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
          <h2 class="text-base/7 font-semibold text-high">Fleet Activity</h2>
          <a href="#" class="text-sm/6 font-semibold text-primary">View all</a>
        </header>
        <ul role="list" class="divide-y divide-line">
          <li v-for="item in activityItems" :key="item.dateTime" class="px-4 py-4 sm:px-6 lg:px-8">
            <div class="flex items-center gap-x-3">
              <img :src="item.operator.imageUrl" alt="" class="size-6 flex-none rounded-full bg-surface-2 outline -outline-offset-1 outline-line" />
              <h3 class="flex-auto truncate text-sm/6 font-semibold text-high">{{ item.operator.name }}</h3>
              <time :datetime="item.dateTime" class="flex-none text-xs text-[--color-gray-6]">{{ item.date }}</time>
            </div>
            <p class="mt-3 truncate text-sm text-low">
              {{ item.action }} on <span class="font-mono text-high">{{ item.vehicleName }}</span> — <span class="text-high">{{ item.detail }}</span>
            </p>
          </li>
        </ul>
      </aside>
